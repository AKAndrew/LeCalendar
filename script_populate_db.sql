...
DECLARE
  ...
  list_city varr := varr('Iasi','Bucuresti','Suceava','Paris','Cluj','Dublin','Londra','Skopje');
...
--        v_password := DBMS_CRYPTO.HASH(rawtohex(v_username) ,2);
        ...
        v_city:= list_city(TRUNC(DBMS_RANDOM.VALUE(0,list_city.count) + 1));

        insert into users values(v_i, v_username, v_password,v_city, sysdate, sysdate);
    END LOOP;
